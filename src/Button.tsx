...
export function Button(pro: Pros) {
    ...
    const firsts = pro.first.map(item => (
        <span className="magic-key" id={item} key={item}>{item}</span>
    ));

    const seconds = pro.second ? pro.second.map(item => (
        <span className="magic-key" key={item}>{item}</span>
    )) : [];
    ...
    return (
        <div className={divClass} id={code}>
